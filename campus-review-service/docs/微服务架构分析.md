...
| 7 | **risk-control-service** | 8087 | 内容审核 (敏感词过滤)、限流判定 (滑动窗口) | 基础设施服务 |
| 8 | **admin-service** | 8088 | 后台统一管理入口、用户/餐厅管理编排 | 支撑服务 (BFF 层) |
...
### 2.7 risk-control-service (风控服务)

| 接口路径 | 方法 | 调用类型 | 说明 |
|----------|------|----------|------|
| `/api/risk/audit` | POST | 内部调用 | 内容审核 (敏感词检测) |
| `/api/risk/ratelimit?key=&limit=&windowSeconds=` | GET | 内部调用 | 限流判定 |

### 2.8 admin-service (后台管理服务)
...
| **risk-control-service** | ⭐⭐⭐ 中 | 敏感词审核应本地调用（延迟敏感），限流应在 API 网关层处理 | 拆除为工具类，整合到网关或各服务本地 |
